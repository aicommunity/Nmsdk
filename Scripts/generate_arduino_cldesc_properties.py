...
PROPS: dict[str, list[tuple[str, str, str, tuple[str, str]]]] = {
    "ArduinoBoard": [
        ...
        ("CancelUpload", "Cancel upload", "Edge: отменить async upload (kill avrdude).", EDGE),
        ...
        ("ConnectionState", "Connection state", "State: 0=Disconnected, 1=Opening, 2=Connected, 3=Error.", STATE),
        ("UploadProgress", "Upload progress", "State: 0..100 прогресс прошивки.", STATE),
        ("UploadLastResult", "Upload last result", "State: `ok` / `uploading` / текст ошибки.", STATE),
        ("LastError", "Last error", "State: текст последней ошибки порта/upload.", STATE),
        ("LastActivityMs", "Last activity ms", "State: время последней активности (ms).", STATE),
        ("MissedHeartbeats", "Missed heartbeats", "State: пропуски heartbeat.", STATE),
        ("RequestHealthCheck", "Request health check", "State/pulse: запрос health check.", STATE),
        ("PortName", "Port name", "Путь к serial (`/dev/ttyACM0`, `COM3`).", PARAM),
        ("BaudRate", "Baud rate", "Скорость serial (по умолчанию 57600).", PARAM),
        ("BoardProfile", "Board profile", "0=Uno, 1=Mega 2560. CH340-клоны: задавайте вручную (AU-10).", PARAM),
        ("AutoReconnect", "Auto reconnect", "Автопереподключение при обрыве.", PARAM),
        ("ConnectOnBuild", "Connect on build", "Автоподключение при `ABuild`, если `PortName` задан.", PARAM),
        ("HeartbeatEnabled", "Heartbeat enabled", "Включить heartbeat.", PARAM),
        ("HeartbeatIntervalMs", "Heartbeat interval ms", "Интервал heartbeat.", PARAM),
        ("HeartbeatTimeoutMs", "Heartbeat timeout ms", "Таймаут heartbeat.", PARAM),
        ("BundledFirmwareId", "Bundled firmware", "ID из manifest (`sensor_lab_v1`, `standard_firmata`).", PARAM),
        ("FirmwarePath", "Firmware path", "Путь к HEX (пусто = bundled id).", PARAM),
        ("ShowDebug", "Show debug", "Отладочный вывод serial session.", PARAM),
    ],
    "ArduinoSensorSketch": [
    ...
    "ArduinoCustomFirmware": [
        ...
        ("ClearFrameLog", "Clear frame log", "Edge: очистить FrameLog.", EDGE),
        ("HostPluginId", "Host plugin id", "Override catalog hostPlugin.", PARAM),
        ("FrameLog", "Frame log", "Ring buffer RX/frames.", STATE),
        ("NamedValuesJson", "Named values JSON", "Bag publishNamedFloat.", STATE),
        ("PluginBound", "Plugin bound", "Найден protocol plugin.", STATE),
        ("Command", "Command", "Текстовая команда.", PARAM),
        ("BundledFirmwareId", "Bundled firmware", "Id прошивки каталога.", PARAM),
    ],
    "ArduinoAdc": [

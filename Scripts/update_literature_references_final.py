...
# -*- coding: utf-8 -*-
"""
Финальный скрипт для переработки ссылок на литературу в md файлах.
...
# Расширенный словарь замен для общих паттернов (в порядке приоритета - более специфичные сначала)
replacements = [
    # Диссертации и ВКР
    ...
    (r'Korsakov A\.M\., Isakov T\.T\., Bakhshiev A\.V\. \(2023\)', '[1]'),
    (r'Korsakov A\., Astapova L\., Bakhshiev A\. \(2023\)', '[4]'),

    # Публикации 2022
    (r'Корсаков и др\. \(2022\)', '[1]'),
    (r'Корсаков А\.М\., Астапова Л\.А\., Бахшиев А\.В\. \(2022\)', '[1]'),
    (r'Korsakov, A\. M\., Astapova, L\. A\., Bakhshiev, A\. V\. \(2022\)', '[1]'),

    # Общие упоминания Бахшиева и Романова
    ...
    # Общие фразы
    (r'связанным публикациям', 'публикациям [1], [13], [31]'),
    (r'связанным публикациям и', 'публикациям [1], [13], [31] и'),
    (r'связанные публикации', 'публикации [1], [13], [31]'),
]

# Шаблоны литературы для разных категорий
...
2. Бахшиев А.В., Романов С.П. Моделирование процессов преобразования импульсных потоков в нейронных структурах управления мышечным сокращением // Нейрокомпьютеры: разработка, применение, №11, 2009. – с.70-79. [онлайн](https://www.elibrary.ru/item.asp?id=13070251)
...
def detect_category(file_path):
    ...
    if 'CableModel' in path_str or 'Cable' in path_str:
        ...
        return 'rcn'
    elif 'MC-Muscles' in path_str or 'MC1-PCN' in path_str or 'EyeRetina' in path_str:
        return 'muscle'
    return None

def clean_broken_references(content):
    """Очищает сломанные ссылки и странные форматы"""
    # Удаляем странные форматы с отдельными строками [номер] и ссылками
    content = re.sub(r'\n\s*\[[0-9A-C]+\]\s*\n\s*\(\[онлайн[^\]]+\]\)\s*\n', '', content)
    content = re.sub(r'\n\s*-\s*\*\*\[[0-9A-C]+\]\*\*\s*\n\s*Раздел.*?\n', '', content, flags=re.MULTILINE)

    # Удаляем сломанные фрагменты типа "- **"The Neuromorphic Model"**- **"Моделирование..."**
    content = re.sub(r'-\s*\*\*"[^"]+"\*\*-\s*\*\*"[^"]+"\*\*', '', content)
    content = re.sub(r'-\s*\*\*"[^"]+"\*\*', '', content)

    # Исправляем сломанные строки с [31] и ссылками
    content = re.sub(r'\s+\[31\]\s*\n\s*\(\[онлайн[^\]]+\]\)\s*\n', '', content)
...
def process_file(file_path):
    ...
    try:
        ...
        # Очищаем сломанные ссылки
        content = clean_broken_references(content)
        ...
        if '## Литература' not in content and has_references:
            ...
            if re.search(r'\[[0-9A-C]\]', content):
                ...
                elif '[A]' in content or 'диссертац' in content.lower():
                    # Для файлов с диссертацией
                    if 'rcn' in str(file_path).lower():
                        content = content.rstrip() + '\n\n' + literature_templates['rcn']
                    else:
                        # Базовый шаблон с диссертацией
                        lit = """## Литература
...
def main():
    ...
    docs_dir = Path('Bin/Docs')
    count = 0

    # Обрабатываем все README.md файлы
    for readme_file in list(base_dir.rglob('README.md')) + list(docs_dir.rglob('*.md')):
        if 'Literature-References' in str(readme_file):
            continue

...
# Fallback for older vcpkg/cmake combos where VERSION may be unset in this context.
# We know from the manifest/baseline that Boost is 1.82.0; keeping this in sync
# with the vcpkg baseline avoids Jamroot.jam having an empty BOOST_VERSION.
...
function(boost_modular_build)
    ...
    if(NOT DEFINED B2_REQUIREMENTS)
        set(B2_REQUIREMENTS "")
    endif()
    ...
    # Ensure PYTHON_VERSION_TAG is defined even if Python is not used
    if(NOT DEFINED PYTHON_VERSION_TAG)
        set(PYTHON_VERSION_TAG "")
    endif()

    # Use overlay template from installed boost-build directory (copied by portfile.cmake)
    # If overlay template exists, it will be in BOOST_BUILD_INSTALLED_DIR/share/boost-build/
    # Otherwise fall back to standard vcpkg template
    set(_JAMROOT_TEMPLATE "${BOOST_BUILD_INSTALLED_DIR}/share/boost-build/Jamroot.jam.in")
    if(NOT EXISTS "${_JAMROOT_TEMPLATE}")
        message(FATAL_ERROR "Jamroot.jam.in template not found at ${_JAMROOT_TEMPLATE}")
    endif()

    configure_file("${_JAMROOT_TEMPLATE}" "${_bm_SOURCE_PATH}/Jamroot.jam" @ONLY)
    ...
    # Ensure boostcpp.jam is available next to the Boost library Jamroot.
    # Without this, b2 may fail with:
    #   [errno 2] boostcpp.jam (No such file or directory)
    # when processing `import boostcpp ;` in the generated Jamroot.jam.
    set(_BOOSTCP_JAM "${BOOST_BUILD_INSTALLED_DIR}/tools/boost-build/boostcpp.jam")
    if(EXISTS "${_BOOSTCP_JAM}")
        file(COPY "${_BOOSTCP_JAM}" DESTINATION "${_bm_SOURCE_PATH}")
    endif()
    ...
    vcpkg_cmake_configure(
        ...
        OPTIONS
            ...
            "-DBOOST_LIB_RELEASE_SUFFIX=${BOOST_LIB_RELEASE_SUFFIX}"
            "-DBOOST_LIB_DEBUG_SUFFIX=${BOOST_LIB_DEBUG_SUFFIX}"
            "-DPYTHON_VERSION_TAG=${PYTHON_VERSION_TAG}"
            ${configure_options}
    ...
    foreach(LIB IN LISTS INSTALLED_LIBS)
        ...
        string(REPLACE "-vc144-" "-vc140-" NEW_FILENAME ${NEW_FILENAME}) # MSVC 14.44 (v144) -> vc140 naming

...
bool NGraphics::SetNet(NANet *net)
{
 if(Net == net)
  return true;
...
/*
 ...
 while(I != J)
 {
  ...
  if(dI == Descriptions.end())
  {
   ...
   if(dynamic_cast<NANet*>(Net->GetComponent(I->second)))
   {
...
bool NGraphics::DrawLinks(const UId &id, bool links, bool highlight, bool contour)
{
 ...
 for(size_t i=0;i<Net->GetNumComponents();i++)
 {
  ...
  {
   item->GetFullItemLinks(linkslist, dynamic_cast<NAItem*>(Net->GetComponentByIndex(i)),Net);
...
void NGraphics::Paint(NVDescription &ndescr, bool highlight)
{
 ...
 PaintIndicator(ndescr.X-ndescr.Radius,ndescr.Y+ndescr.Radius+ndescr.ContourWidth, ndescr.Radius*2, ndescr.Radius/2,
				   ndescr.TopIndicator, 1, IndicatorFullColorTop, IndicatorEmptyColorTop);
 PaintIndicator(ndescr.X-ndescr.Radius,ndescr.Y-ndescr.Radius-ndescr.ContourWidth*3, ndescr.Radius*2, ndescr.Radius/2,
				   ndescr.BottomIndicator, 1, IndicatorFullColorBottom, IndicatorEmptyColorBottom);
 PaintIndicator(ndescr.X-ndescr.Radius-ndescr.Radius/2-ndescr.ContourWidth,ndescr.Y-ndescr.Radius, ndescr.Radius/2, ndescr.Radius*2,
				   ndescr.LeftIndicator, 3, IndicatorFullColorLeft, IndicatorEmptyColorLeft);
 PaintIndicator(ndescr.X+ndescr.Radius+ndescr.ContourWidth,ndescr.Y-ndescr.Radius, ndescr.Radius/2, ndescr.Radius*2,
				   ndescr.RightIndicator, 3, IndicatorFullColorRight, IndicatorEmptyColorRight);
...
void NGraphics::PaintItem(NVDescription &ndescr, bool highlight)
{
 ...
 else
 {
  if(highlight)
  {
   ...
   GEngine->SetPenColor(ndescr.ContourColor);
   GEngine->Circle(ndescr.X,ndescr.Y,ndescr.Radius-(ndescr.ContourWidth+ndescr.ContourWidth/2-2),true);
  }
  else
  {
   GEngine->SetPenColor(ndescr.ContourColor);
...
void NGraphics::PaintNet(NVDescription &ndescr, bool highlight)
{
 ...
 else
 {
  if(highlight)
  {
   ...
   GEngine->SetPenColor(ndescr.FillColor);
   GEngine->Circle(ndescr.X,ndescr.Y,ndescr.Radius-ndescr.ContourWidth,true);
  }
  else
  {
   GEngine->SetPenColor(ndescr.FillColor);
				   ...
				   double precent, int direction, UColorT full, UColorT empty)
{
 ...
 {
 case 1:
  GEngine->Rect(x,y,x+static_cast<int>((width*precent)/100.0),y+height,true);
 break;

 case 2:
  GEngine->Rect(x+static_cast<int>((width-(width*precent)/100.0)),y,x+width,y+height,true);
 break;

 case 3:
  GEngine->Rect(x,y,x+width,y+static_cast<int>((height*precent)/100.0),true);
 break;

 case 4:
  GEngine->Rect(x,y+static_cast<int>((height-(height*precent)/100.0)),x+width,y+height,true);
 break;
 }
}
			   ...
			   bool links, bool highlight, bool contour)
{
 ...
 if(links)
 {
  ...
  GEngine->Rect(in.X-in.Radius-out.LinkWidth,in.Y-out.LinkWidth,

...
TEST_F(ModelConnectionsTest, CreateLinks) {
    ...
    auto found1 = model->GetComponent("Component1", true);
    auto found2 = model->GetComponent("Component2", true);
    EXPECT_NE(found1, nullptr) << "Component1 not found in model";
    EXPECT_NE(found2, nullptr) << "Component2 not found in model";
}
...
TEST_F(ModelConnectionsTest, CheckLinks) {
    ...
    auto owner1 = component1->GetMainOwner();
    auto owner2 = component2->GetMainOwner();
    ...
    EXPECT_NE(model->GetComponent("Component1", true), nullptr);
    EXPECT_NE(model->GetComponent("Component2", true), nullptr);
}
...
TEST_F(ModelConnectionsTest, LinkStructure) {
    ...
    if (component) {
        ...
        EXPECT_NE(model->GetComponent("TestComponent", true), nullptr) << "Component should be findable";
    }
}
...
TEST_F(ModelConnectionsTest, BreakLinks) {
    ...
    EXPECT_EQ(found, nullptr) << "Component should not be found after removal";
}

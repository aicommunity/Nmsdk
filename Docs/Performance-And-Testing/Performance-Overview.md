...
В коде это реализовано через кэш `CachedConnectedOutput` в шаблонных классах свойств (`UVProperty`, `UVBaseDataProperty`), который хранит типизированный указатель на подключённое выходное свойство, избегая повторных `dynamic_cast` и поиска в `ConnectedOutputs`.
...
В `UContainer` реализован кэш `ActiveComponentsCache`, который обновляется только при изменении активности компонентов (через `SetActivity`) или при добавлении/удалении компонентов. Метод `GetActiveComponents()` возвращает кэшированный список вместо фильтрации всех компонентов каждый раз.
...
В `UNet` и `UContainer` используется топологическая сортировка для определения порядка выполнения компонентов на основе их связей через свойства. Это гарантирует, что компоненты выполняются только после того, как все их входные данные готовы, минимизируя задержки и повторные вычисления.
